...
from legged_gym.algorithm.mlp_encoder import MLP_Encoder, TemporalContextEncoder
...
def export_policy_as_onnx(args, robot_type):
    ...
    encoder_model = copy.deepcopy(encoder).to("cpu")
    encoder_model.eval()
    ...
    critic_input_dim = (
        env_cfg.env.num_critic_observations
        + env_cfg.commands.num_commands
        + getattr(encoder, "num_critic_output_dim", encoder.num_output_dim)
    )
